...
layout(location = 1) in vec2 v_pos_px;
layout(location = 2) in vec4 v_clip_rect_px;
layout(location = 3) in float v_clip_radius_px;
layout(location = 4) in float v_clip_mode;
...
void main() {
    float alpha = clip_alpha(v_pos_px, v_clip_rect_px, v_clip_radius_px, v_clip_mode);
    if (alpha <= 0.001) {
        discard;
    }
