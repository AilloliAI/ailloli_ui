...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if warmups != LOCKED_WARMUPS || measured < MIN_MEASURED {
        return Err(format!(
            "Phase 127 requires exactly {LOCKED_WARMUPS} warmups and at least {MIN_MEASURED} measured samples"
        )
        .into());
    }
    ...
    let BenchInit::Enabled(session) = &bench else {
        return Err("set AILLOLI_UI_BENCH=1 for the Phase 127 harness".into());
    };
    ...
    match scenario.as_str() {
        ...
        _ => return Err(format!("unsupported Phase 127 framework scenario {scenario:?}").into()),
    }
...
fn update_metadata(
    ...
    metadata.harness = Some("phase127_regression_bench".to_string());
...
fn run_tree_virtualization(
    ...
    let tree_id = runtime
        .tree
        .resolve_element_by_view_key("phase127-bench-tree")
