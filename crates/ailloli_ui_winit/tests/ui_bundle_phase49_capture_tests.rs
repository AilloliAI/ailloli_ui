//! Local-only Phase 49 visual tests for Sidebar, NavItem, ListView and DisclosureRow.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_bundle_phase49_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_navigation_lists_showcase, ShowcaseMode};
...
#[test]
#[ignore]
fn ui_bundle_phase49_navigation_lists_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-navigation-lists");
    let white_id = cap.request_element("white", "section-navigation-lists");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_phase49_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_navigation_lists_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_phase49_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_navigation_lists_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default navigation/list section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white navigation/list section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_phase49_navigation_lists.png");
    assert_non_empty_frame(&white, "ui_bundle_phase49_navigation_lists_white.png");
    assert_non_monochrome(&default, "ui_bundle_phase49_navigation_lists.png");
    assert_non_monochrome(&white, "ui_bundle_phase49_navigation_lists_white.png");
    write_capture("ui_bundle_phase49_navigation_lists.png", &default);
    write_capture("ui_bundle_phase49_navigation_lists_white.png", &white);
