//! UI-XR 20 visual proof for the reconciled upstream widget and renderer paths.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit \
//!   --test ui_xr20_upstream_merge_capture_tests \
//!   ui_xr20_upstream_merge_capture \
...
/// Resolves the repository-local directory used for XR20 captures.
...
fn assert_ui_xr20_frame(frame: &CapturedFrame) {
...
#[test]
#[ignore]
fn ui_xr20_upstream_merge_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let capture_id = cap.request_element("ui-xr20-upstream-merge", "ui-xr20-window");

    App::new()
        .window(
            Window::new("ui-xr20-upstream-merge")
                .title_text("ui_xr20_upstream_merge")
                .no_chrome()
                .size(1180.0, 760.0)
                .content(ui_xr20_showcase),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let frame = cap
        ...
        .expect("ui-xr20 capture slot")
        .expect("ui-xr20 capture ok")
        .frame;

    assert_ui_xr20_frame(&frame);
    write_capture(&frame);
}

/// Builds the complete upstream-merge validation board.
fn ui_xr20_showcase() -> impl IntoView<()> {
    Container::<()>::new()
        .fill()
        .background(Color::rgb(4, 9, 18))
        .padding(34.0)
        .child(
            WindowAffordanceFrame::<()>::new("UI-XR 20 • Upstream Reconciled")
                .logical_window_id("ui-xr20-window")
                .width(1070.0)
                .height(650.0)
