//! Local-only Phase 54.1 visual tests for TerminalView.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_bundle_phase54_1_capture_tests -- --ignored --nocapture
//! ```
...
#[test]
#[ignore]
fn ui_bundle_phase54_1_terminal_view_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("terminal-phase54-1-suite", "section-terminal-view");
    let search_id = cap.request_element("terminal-phase54-1-suite", "section-terminal-view-search");
    let selection_id = cap.request_element(
        "terminal-phase54-1-suite",
        "section-terminal-view-selection",
    );

    App::new()
        .window(
            Window::new("terminal-phase54-1-suite")
                .title_text("ui_bundle_phase54_1_terminal_view")
                .no_chrome()
                .size(1280.0, 430.0)
                .content(|| {
                    ui_bundle_terminal_phase54_1_capture_suite_showcase(ShowcaseMode::DefaultTheme)
                }),
        )
        .capture(cap.clone())
        .run()
    ...
    assert_terminal_frame(&default, "ui_bundle_phase54_1_terminal_view.png");
    assert_terminal_frame(&search, "ui_bundle_phase54_1_terminal_view_search.png");
    assert_terminal_frame(
        &selection,
        "ui_bundle_phase54_1_terminal_view_selection.png",
    );
    write_capture("ui_bundle_phase54_1_terminal_view.png", &default);
    write_capture("ui_bundle_phase54_1_terminal_view_search.png", &search);
    write_capture(
        "ui_bundle_phase54_1_terminal_view_selection.png",
        &selection,
    );
