//! Local-only CodeEditor gutter clipping visual test for CodeEditor gutter clipping.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test code_editor_gutter_clipping_capture_tests -- --ignored --nocapture
//! ```
...
#[test]
#[ignore]
fn ui_bundle_code_editor_gutter_clipping_code_editor_gutter_clip_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let capture_id = cap.request_element(
        "code-editor-gutter-clipping-gutter-clip",
        "code-editor-gutter-clipping-code-editor-widget",
    );

    App::new()
        .window(
            Window::new("code-editor-gutter-clipping-gutter-clip")
                .title_text("ui_bundle_code_editor_gutter_clipping_code_editor_gutter_clip")
                .no_chrome()
                .size(760.0, 340.0)
    ...
    assert_gutter_clip_frame(
        &frame,
        "ui_bundle_code_editor_gutter_clipping_code_editor_gutter_clip.png",
    );
    write_capture(
        "ui_bundle_code_editor_gutter_clipping_code_editor_gutter_clip.png",
        &frame,
    ...
    Container::new()
        ...
        .child(
            CodeEditor::new(document)
                ...
                .height(280.0)
                .into_view()
                .key("code-editor-gutter-clipping-code-editor-widget"),
        )
}
