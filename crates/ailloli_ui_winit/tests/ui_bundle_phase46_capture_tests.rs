//! Local-only Phase 46 visual tests for Select and Dropdown.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_bundle_phase46_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_select_dropdown_showcase, ShowcaseMode};
...
#[test]
#[ignore]
fn ui_bundle_phase46_select_dropdown_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-select-dropdowns");
    let white_id = cap.request_element("white", "section-select-dropdowns");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_phase46_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_select_dropdown_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_phase46_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_select_dropdown_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default select dropdown section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white select dropdown section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_phase46_select_dropdown.png");
    assert_non_empty_frame(&white, "ui_bundle_phase46_select_dropdown_white.png");
    assert_non_monochrome(&default, "ui_bundle_phase46_select_dropdown.png");
    assert_non_monochrome(&white, "ui_bundle_phase46_select_dropdown_white.png");
    write_capture("ui_bundle_phase46_select_dropdown.png", &default);
    write_capture("ui_bundle_phase46_select_dropdown_white.png", &white);
    ...
    assert!(white_orange > 30, "white orange pixels={white_orange}");
}
