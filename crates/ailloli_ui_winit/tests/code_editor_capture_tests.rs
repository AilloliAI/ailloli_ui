//! Local-only CodeEditor visual tests for CodeEditor.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test code_editor_capture_tests -- --ignored --nocapture
//! ```
...
/// Requires encoded PNG data and the expected minimum editor-section extent.
...
/// Verifies editor syntax and chrome colors for the selected palette.
...
#[test]
#[ignore]
fn ui_bundle_code_editor_code_editor_capture() {
    ...
    App::new()
        .window(
            Window::new("code-editor-default")
                .title_text("ui_bundle_code_editor_code_editor_default")
                .no_chrome()
                .size(1280.0, 900.0)
        ...
        .window(
            Window::new("code-editor-white")
                .title_text("ui_bundle_code_editor_code_editor_white")
                .no_chrome()
                .size(1280.0, 900.0)
    ...
    assert_code_editor_frame(&default, "ui_bundle_code_editor_code_editor.png", true);
    assert_code_editor_frame(&white, "ui_bundle_code_editor_code_editor_white.png", false);
    write_capture("ui_bundle_code_editor_code_editor.png", &default);
    write_capture("ui_bundle_code_editor_code_editor_white.png", &white);
}
