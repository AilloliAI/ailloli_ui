//! Local-only progress indicators visual tests for ProgressBar and CircularProgress.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test progress_indicators_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_progress_showcase, ShowcaseMode};
...
#[test]
#[ignore]
fn ui_bundle_progress_indicators_progress_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-progress");
    let white_id = cap.request_element("white", "section-progress");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_progress_indicators_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_progress_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_progress_indicators_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_progress_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default progress section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white progress section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_progress_indicators_progress.png");
    assert_non_empty_frame(&white, "ui_bundle_progress_indicators_progress_white.png");
    assert_non_monochrome(&default, "ui_bundle_progress_indicators_progress.png");
    assert_non_monochrome(&white, "ui_bundle_progress_indicators_progress_white.png");
    write_capture("ui_bundle_progress_indicators_progress.png", &default);
    write_capture("ui_bundle_progress_indicators_progress_white.png", &white);
    ...
    assert!(orange > 180, "orange pixels={orange}");
    ...
    assert!(white_orange > 180, "white orange pixels={white_orange}");
}
