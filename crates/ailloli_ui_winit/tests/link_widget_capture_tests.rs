//! Local-only visual proof for link widget Link and text underline rendering.
...
fn link_showcase_component(ctx: &mut Context<()>, _props: ()) -> View<()> {
    ctx.runtime().request_focus_key("link_widget-focused-link");
    ...
    Container::new()
        ...
        .into_view()
        .key("link_widget-link-showcase")
}

#[test]
#[ignore = "requires a native compositor and WGPU capture"]
fn ui_bundle_link_widget_link_capture() {
    let capture = CaptureHandle::new();
    capture.set_exit_after_all_captures(true);
    let capture_id = capture.request_element("link_widget-link", "link_widget-link-showcase");

    App::new()
        .window(
            Window::new("link_widget-link")
                .title_text("ui_bundle_link_widget_link")
                .no_chrome()
                .size(720.0, 360.0)
        ...
        .run()
        .expect("link_widget capture app");

    let frame = capture
        .take_result(capture_id)
        .expect("link_widget capture slot")
        .expect("link_widget capture result")
        .frame;
    ...
    let output = repo_captures_dir().join("ui_bundle_link_widget_link.png");
    ...
    std::fs::write(&output, frame.png_data.as_ref().expect("PNG data"))
        .expect("write link_widget capture");
