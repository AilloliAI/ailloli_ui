//! Local-only terminal widget visual test for the state-backed Terminal widget.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test terminal_widget_capture_tests -- --ignored --nocapture --test-threads=1
//! ```
...
use ui_bundle_showcase::{ui_bundle_terminal_widget_showcase, ShowcaseMode};
...
/// Verifies terminal-widget colors, contrast, extent, and encoded PNG data.
...
#[test]
#[ignore]
fn ui_bundle_terminal_widget_terminal_widget_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let terminal_id = cap.request_element("terminal-widget-suite", "section-terminal-widget-v2");

    App::new()
        .window(
            Window::new("terminal-widget-suite")
                .title_text("ui_bundle_terminal_widget_terminal_widget")
                .no_chrome()
                .size(980.0, 390.0)
                .content(|| ui_bundle_terminal_widget_showcase(ShowcaseMode::DefaultTheme)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    assert_terminal_widget_frame(&frame, "ui_bundle_terminal_widget_terminal_widget.png");
    write_capture("ui_bundle_terminal_widget_terminal_widget.png", &frame);
}
