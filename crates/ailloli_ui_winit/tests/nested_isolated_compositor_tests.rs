...
/// Builds a layer pass and assigns its explicit nesting depth in the render DAG.
fn isolated_layer(
    cmds: &[DrawCmd],
...
#[derive(Default)]
/// Aggregates independently evaluated failures across nested-layer scenarios.
...
fn scenario_s_parent_blur_child_opacity(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact(
            "diag_nested_isolated_compositor_S_parent_blur_child_opacity.png",
            png,
        );
    }
...
fn scenario_t_parent_noop_child_blur(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact(
            "diag_nested_isolated_compositor_T_parent_noop_child_blur.png",
            png,
        );
    }
...
fn scenario_u_three_levels(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    let captured = renderer
        ...
        .expect("capture U");
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact("diag_nested_isolated_compositor_U_three_levels.png", png);
    }
...
fn scenario_v_siblings_in_parent(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact(
            "diag_nested_isolated_compositor_V_siblings_in_parent.png",
            png,
        );
    }
...
#[test]
#[ignore]
fn nested_isolated_compositor_nested_isolated_visual_regressions() {
    ...
    assert!(
        ...
        "nested_isolated_compositor nested isolated scenarios failed:\n  - {}",
