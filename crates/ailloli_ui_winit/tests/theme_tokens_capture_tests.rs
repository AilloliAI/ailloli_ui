//! Local-only theme tokens visual test for the default theme tokens.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test theme_tokens_capture_tests -- --ignored --nocapture
//! ```
...
fn theme_tokens_board(primary: State<String>, filled: State<String>) -> impl IntoView<()> {
    let theme = Theme::default();
        ...
        .child(
            ScrollView::vertical().child(
                Column::new()
                    ...
                    .padding(24.0)
                    .child(
                        Text::new("Ailloli UI theme tokens Theme Tokens")
...
#[test]
#[ignore]
fn ui_bundle_theme_tokens_theme_tokens_capture() {
    ...
    App::new()
        .window(
            Window::new("main")
                .title_text("ui_bundle_theme_tokens_theme_tokens")
                .no_chrome()
                .size(980.0, 680.0)
                .content(move || theme_tokens_board(primary.clone(), filled.clone())),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let out_file = out_dir.join("ui_bundle_theme_tokens_theme_tokens.png");
