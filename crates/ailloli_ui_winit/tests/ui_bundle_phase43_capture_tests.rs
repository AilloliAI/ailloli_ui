//! Local-only Phase 43 visual tests for SegmentedControl.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_bundle_phase43_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_segmented_control_showcase, ShowcaseMode};
...
#[test]
#[ignore]
fn ui_bundle_phase43_segmented_control_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-segmented-controls");
    let white_id = cap.request_element("white", "section-segmented-controls");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_phase43_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_segmented_control_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_phase43_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_segmented_control_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default segmented section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white segmented section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_phase43_segmented_control.png");
    assert_non_empty_frame(&white, "ui_bundle_phase43_segmented_control_white.png");
    assert_non_monochrome(&default, "ui_bundle_phase43_segmented_control.png");
    assert_non_monochrome(&white, "ui_bundle_phase43_segmented_control_white.png");
    write_capture("ui_bundle_phase43_segmented_control.png", &default);
    write_capture("ui_bundle_phase43_segmented_control_white.png", &white);
