//! Local-only RadioGroup visual tests for RadioGroup.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test radio_group_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_radio_group_showcase, ShowcaseMode};
...
#[test]
#[ignore]
fn ui_bundle_radio_group_radio_group_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-radio-groups");
    let white_id = cap.request_element("white", "section-radio-groups");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_radio_group_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_radio_group_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_radio_group_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_radio_group_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default radio section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white radio section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_radio_group_radio_group.png");
    assert_non_empty_frame(&white, "ui_bundle_radio_group_radio_group_white.png");
    assert_non_monochrome(&default, "ui_bundle_radio_group_radio_group.png");
    assert_non_monochrome(&white, "ui_bundle_radio_group_radio_group_white.png");
    write_capture("ui_bundle_radio_group_radio_group.png", &default);
    write_capture("ui_bundle_radio_group_radio_group_white.png", &white);
    ...
    assert!(orange > 120, "orange pixels={orange}");
    ...
    assert!(white_orange > 120, "white orange pixels={white_orange}");
}
