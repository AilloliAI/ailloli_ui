//! Local-only Card, Avatar, StatusIndicator and Divider visual tests for Card, Avatar, StatusIndicator and Divider.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test card_avatar_status_divider_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_cards_avatar_status_divider_showcase, ShowcaseMode};
...
fn assert_non_empty_frame(frame: &CapturedFrame, name: &str) {
    ...
    assert!(frame.width > 240, "{name}: width={}", frame.width);
...
#[test]
#[ignore]
fn ui_bundle_card_avatar_status_divider_cards_avatar_status_divider_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-cards-avatar-status-divider");
    let white_id = cap.request_element("white", "section-cards-avatar-status-divider");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_card_avatar_status_divider_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| {
                    ui_bundle_cards_avatar_status_divider_showcase(ShowcaseMode::DefaultTheme)
                }),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_card_avatar_status_divider_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_cards_avatar_status_divider_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default cards/avatar/status/divider section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white cards/avatar/status/divider section capture ok")
        .frame;

    assert_non_empty_frame(
        &default,
        "ui_bundle_card_avatar_status_divider_cards_avatar_status_divider.png",
    );
    assert_non_empty_frame(
        &white,
        "ui_bundle_card_avatar_status_divider_cards_avatar_status_divider_white.png",
    );
    assert_non_monochrome(
        &default,
        "ui_bundle_card_avatar_status_divider_cards_avatar_status_divider.png",
    );
    assert_non_monochrome(
    ...
    let green = count_pixels(&default.rgba, |px| {
        px[3] > 140 && px[0] < 90 && px[1] > 130 && px[2] < 140
    });
    let yellow = count_pixels(&default.rgba, |px| {
        px[3] > 140 && px[0] > 170 && px[1] > 100 && px[2] < 80
    });
    let danger = count_pixels(&default.rgba, |px| {
        px[3] > 140 && px[0] > 180 && px[1] < 130 && px[2] < 130 && px[0] > px[1].saturating_add(60)
    });
    ...
    assert!(orange > 20, "orange pixels={orange}");
    assert!(green > 8, "green pixels={green}");
    assert!(yellow > 8, "yellow pixels={yellow}");
    assert!(danger > 20, "danger pixels={danger}");
    assert!(dark_surface > 2_000, "dark surface pixels={dark_surface}");
    assert!(light_text > 120, "light text pixels={light_text}");
