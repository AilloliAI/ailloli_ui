//! Local-only terminal scrollback and TUI visual tests for Terminal scrollback/selection and TUI modes.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test terminal_scrollback_tui_capture_tests -- --ignored --nocapture --test-threads=1
//! ```
...
use ui_bundle_showcase::{ui_bundle_terminal_scrollback_tui_showcase, ShowcaseMode};
...
/// Verifies terminal colors and optionally requires a visible selection highlight.
...
#[test]
#[ignore]
fn ui_bundle_terminal_scrollback_tui_terminal_capture_suite() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let scrollback_id = cap.request_element(
        "terminal-scrollback-tui-suite",
        "section-terminal-scrollback-selection",
    );
    let tui_id = cap.request_element("terminal-scrollback-tui-suite", "section-terminal-tui");

    App::new()
        .window(
            Window::new("terminal-scrollback-tui-suite")
                .title_text("ui_bundle_terminal_scrollback_tui_terminal")
                .no_chrome()
                .size(980.0, 650.0)
                .content(|| ui_bundle_terminal_scrollback_tui_showcase(ShowcaseMode::DefaultTheme)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    assert_terminal_frame(
        &scrollback,
        "ui_bundle_terminal_scrollback_tui_terminal_scrollback.png",
        true,
    );
    assert_terminal_frame(
        &tui,
        "ui_bundle_terminal_scrollback_tui_terminal_tui.png",
        false,
    );
    write_capture(
        "ui_bundle_terminal_scrollback_tui_terminal_scrollback.png",
    ...
    write_capture("ui_bundle_terminal_scrollback_tui_terminal_tui.png", &tui);
}
