//! Native visual proof for the Phase 125 host boundary and retained lifecycle.
//!
    ...
    Container::new()
        ...
        .child(
            Column::new()
                .fill_width()
                .gap(16.0)
                .child(Text::new("Phase 125 — provider-neutral input").size(22.0))
                .child(
                    Row::new()
                        .gap(14.0)
                        ...
                        .child(
                            Button::with_label("Action button")
                                .on_click(())
                                .into_view()
                                .key("phase125-action-button"),
                        )
                        .child(
                            Link::with_label("Documentation")
                                .href("https://docs.ailloli.ai")
                                .into_view()
                                .key("phase125-documentation-link"),
                        )
                        .child(
    ...
    Container::new()
        ...
        .child(
            Column::new()
                .fill_width()
                .gap(20.0)
                .child(Text::new("Phase 125 — retained popup portal + overlay fallback").size(22.0))
                .child(
                    Row::new()
                        .gap(260.0)
                ...
                .child(
                    Row::new()
                        ...
                        .child(
                            ...
                            .into_view()
                            .key("phase125-tooltip-trigger"),
                        ),
                ),
        )
        .into_view()
        .key("phase125-popup-scene")
}
    ...
    Container::new()
        ...
        .child(
            Column::new()
                .fill_width()
                .gap(16.0)
                .child(Text::new("Phase 125 — recovered native presentation").size(22.0))
                .child(Text::new(
                ...
                .child(
                    ContextMenu::<()>::new(Button::with_label("Popup still operational"))
                        ...
                        .into_view()
                        .key("phase125-recovery-context-menu"),
                )
                .child(
...
fn assert_visual_frame(frame: &CapturedFrame, label: &str) {
    ...
    let required_regions: &[(u32, u32, u32, u32)] = match label {
        "ui_bundle_phase125_input_regression.png" => {
            &[(20, 66, 360, 44), (20, 118, 860, 40), (20, 166, 860, 198)]
        }
        "ui_bundle_phase125_popup_fallback.png" => &[
            (20, 70, 240, 145),
            (20, 286, 500, 138),
            (570, 286, 285, 100),
        ],
        "ui_bundle_phase125_surface_recovery.png" => {
...
fn ui_bundle_phase125_winit_architecture_capture() {
    ...
    let ui = UiApp::new()
        ...
        .window(
            window_options(INPUT_WINDOW, "Phase 125 input", 900.0, 520.0),
            component(input_props, input_scene),
        )
        .window(
            window_options(POPUP_WINDOW, "Phase 125 popups", 980.0, 620.0),
            component((), popup_scene),
        )
        .window(
            window_options(RECOVERY_WINDOW, "Phase 125 recovery", 860.0, 500.0),
    ...
    let mut host = WinitHost::new(ui, NoopHostDriver).test_service(move |ui| {
        ...
        if recovery_popup_stage == 0 {
            let trigger = ui
                .presentation_test_element_bounds(
                    &recovery_id_for_service,
                    "phase125-recovery-context-menu",
                )
        ...
        if popup_visual_stage == 0 {
            ...
            let tooltip_bounds = ui
                .presentation_test_element_bounds(&popup_id_for_service, "phase125-tooltip-trigger")
        ...
        let action_bounds = ui
            .presentation_test_element_bounds(&input_id_for_service, "phase125-action-button")
        ...
        let link_bounds = ui
            .presentation_test_element_bounds(&input_id_for_service, "phase125-documentation-link")
        ...
        let input_bounds = ui
            .presentation_test_element_bounds(&input_id_for_service, "phase125-focused-input")
        ...
        assert_eq!(
            ui.presentation_test_focus_within_key(&input_id_for_service, "phase125-focused-input"),
            Some(true),
            "pointer click must focus the Phase 125 TextInput"
        );
        assert!(ui.inject_event_envelope(window_ime_commit(
            76,
            INPUT_WINDOW,
        ...
        let editor_bounds = ui
            .presentation_test_element_bounds(&input_id_for_service, "phase125-editor")
        ...
        assert_eq!(
            ui.presentation_test_focus_within_key(&input_id_for_service, "phase125-editor"),
            Some(true),
    ...
    run_winit_host(&mut host).expect("run Phase 125 native capture host");
    if let Some(error) = host.take_error() {
        panic!("Phase 125 native capture failed: {error}");
    }
    ...
    assert_eq!(
        host.ui()
            .presentation_test_focus_within_key(&recovery_id, "phase125-recovery-input"),
        Some(true),
    ...
    let context_trigger = host
        .ui()
        .presentation_test_element_bounds(&popup_id, "phase125-context-menu")
    ...
    for (id, name) in [
        (input_capture, "ui_bundle_phase125_input_regression.png"),
        (popup_capture, "ui_bundle_phase125_popup_fallback.png"),
        (recovery_capture, "ui_bundle_phase125_surface_recovery.png"),
    ] {
        let frame = capture
            .take_result(id)
            .expect("Phase 125 capture slot")
            .expect("Phase 125 capture result")
            .frame;
