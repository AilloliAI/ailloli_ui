//! Local-only Phase 50 visual tests for Accordion and TreeView.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_bundle_phase50_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_accordion_tree_showcase, ShowcaseMode};
...
#[test]
#[ignore]
fn ui_bundle_phase50_accordion_tree_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-accordion-tree");
    let white_id = cap.request_element("white", "section-accordion-tree");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_phase50_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_accordion_tree_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_phase50_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_accordion_tree_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default accordion/tree section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white accordion/tree section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_phase50_accordion_tree.png");
    assert_non_empty_frame(&white, "ui_bundle_phase50_accordion_tree_white.png");
    assert_non_monochrome(&default, "ui_bundle_phase50_accordion_tree.png");
    assert_non_monochrome(&white, "ui_bundle_phase50_accordion_tree_white.png");
    write_capture("ui_bundle_phase50_accordion_tree.png", &default);
    write_capture("ui_bundle_phase50_accordion_tree_white.png", &white);
    ...
    assert!(white_orange > 0, "white orange pixels={white_orange}");
}
