//! Native visual proof for the tree virtualization retained and viewport-bounded TreeView.
...
const WINDOW_ID: &str = "tree_virtualization-tree-virtualization";
...
fn ui_bundle_tree_virtualization_tree_virtualization_capture() {
    ...
    let ui = UiApp::new().capture_handle(capture.clone()).window(
        WindowOptions {
            ...
            title: "tree virtualization virtual TreeView".to_string(),
            decorations: false,
            ..Default::default()
        }
    ...
    run_winit_host(&mut host).expect("run tree virtualization TreeView capture host");
    if let Some(error) = host.take_error() {
        panic!("tree virtualization TreeView capture failed: {error}");
    }
    let id = capture_id
        .lock()
    ...
    .expect("write tree virtualization TreeView capture");
