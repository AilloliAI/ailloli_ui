...
//!
//! They guard against:
//!   - the multilayer compositing trap (shared vertex buffer rewritten before submit),
...
fn scenario_e_8_layers(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact(
            "diag_single_pass_compositing_E_eight_layers_alternating.png",
            png,
        );
    }
...
fn scenario_f_stencil_nested_rects_post(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact(
            "diag_single_pass_compositing_F_stencil_nested_rects_post.png",
            png,
        );
    }
...
fn scenario_g_arena_packing(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact("diag_single_pass_compositing_G_arena_packing.png", png);
    }
...
#[test]
#[ignore]
fn single_pass_compositing_single_pass_visual_regressions() {
    ...
    assert!(
        ...
        "single_pass_compositing single-pass scenarios failed:\n  - {}",
