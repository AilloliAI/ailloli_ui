//! split pane visual test for ResizeBar / SplitPane.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test split_pane_capture_tests -- --ignored --nocapture --test-threads=1
//! ```
...
fn assert_split_pane_frame(frame: &CapturedFrame, name: &str) {
...
#[test]
#[ignore]
fn ui_bundle_split_pane_resize_bar_split_pane_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let capture_id = cap.request_element("split_pane-resize-split", "split_pane-resize-split");

    App::new()
        .window(
            Window::new("split_pane-resize-split")
                .title_text("ui_bundle_split_pane_resize_bar_split_pane")
                .no_chrome()
                .size(900.0, 500.0)
                .content(split_pane_showcase),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let frame = cap
        ...
        .expect("split_pane capture slot")
        .expect("split_pane capture ok")
        .frame;

    assert_split_pane_frame(&frame, "ui_bundle_split_pane_resize_bar_split_pane.png");
    write_capture("ui_bundle_split_pane_resize_bar_split_pane.png", &frame);
}

/// Builds horizontal and vertical resize-bar/split-pane fixtures.
fn split_pane_showcase() -> impl IntoView<()> {
