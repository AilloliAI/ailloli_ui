//! UI-XR 17 visual capture for WindowAffordanceFrame.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_xr17_window_affordances_capture_tests -- --ignored --nocapture --test-threads=1
//! ```
...
#[test]
#[ignore]
fn ui_xr17_window_affordances_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let capture_id = cap.request_element("ui-xr17-window-affordances", "ui-xr17-window");

    App::new()
        .window(
            Window::new("ui-xr17-window-affordances")
                .title_text("ui_xr17_window_affordances")
                .no_chrome()
                .size(960.0, 560.0)
                .content(ui_xr17_showcase),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let frame = cap
        ...
        .expect("ui-xr17 capture slot")
        .expect("ui-xr17 capture ok")
        .frame;

    assert_ui_xr17_frame(&frame, "ui_xr17_window_affordances_winit.png");
    ...
    Container::<()>::new()
        ...
        .child(
            WindowAffordanceFrame::<()>::new("VR Slate Controls")
                .logical_window_id("ui-xr17-window")
                .width(780.0)
                .height(410.0)
