//! Local-only EditorPane visual tests for EditorPane.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test editor_pane_capture_tests -- --ignored --nocapture
//! ```
...
/// Resolves the repository-local directory used for editor-pane captures.
...
/// Verifies editor-pane extent, syntax colors, contrast, and encoded PNG data.
fn assert_editor_pane_editor_pane_frame(frame: &CapturedFrame, name: &str) {
...
#[test]
#[ignore]
fn ui_bundle_editor_pane_editor_pane_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let pane_id = cap.request_element("editor_pane-editor-pane", "editor_pane-editor-pane");

    App::new()
        .window(
            Window::new("editor_pane-editor-pane")
                .title_text("ui_bundle_editor_pane_editor_pane")
                .no_chrome()
                .size(960.0, 540.0)
    ...
    assert_editor_pane_editor_pane_frame(&frame, "ui_bundle_editor_pane_editor_pane.png");
    write_capture("ui_bundle_editor_pane_editor_pane.png", &frame);
}

/// Builds the deterministic editor pane with tabs, gutter, and code buffer.
