//! Phase 32 — isolated compositor hardening (budgets, pool reuse, stress).
...
fn scenario_n_siblings(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact("diag_phase32_N_siblings.png", png);
    }
...
fn scenario_q_blur_clamp(renderer: &mut Renderer, rep: &mut ScenarioReport) {
    ...
    let captured = renderer
        ...
        .expect("Q");
    if let Some(png) = captured.png_data.as_ref() {
        write_artifact("diag_phase32_Q_blur_clamp.png", png);
    }
    let m = renderer.isolated_frame_metrics();
...
#[test]
#[ignore]
fn phase32_isolated_hardening_visual_regressions() {
