//! Local-only multiline text input visual test for TextInput multiline.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test multiline_text_input_capture_tests -- --ignored --nocapture --test-threads=1
//! ```
...
fn assert_multiline_text_input_frame(frame: &CapturedFrame, name: &str) {
...
#[test]
#[ignore]
fn ui_bundle_multiline_text_input_text_input_multiline_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let capture_id = cap.request_element(
        "multiline_text_input-text-input-multiline",
        "multiline_text_input-text-input-multiline",
    );

    App::new()
        .window(
            Window::new("multiline_text_input-text-input-multiline")
                .title_text("ui_bundle_multiline_text_input_text_input_multiline")
                .no_chrome()
                .size(860.0, 320.0)
                .content(multiline_text_input_showcase),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let frame = cap
        ...
        .expect("multiline_text_input capture slot")
        .expect("multiline_text_input capture ok")
        .frame;

    assert_multiline_text_input_frame(
        &frame,
        "ui_bundle_multiline_text_input_text_input_multiline.png",
    );
    write_capture(
        "ui_bundle_multiline_text_input_text_input_multiline.png",
        &frame,
...
fn multiline_text_input_seed() -> String {
    [
...
fn multiline_text_input_showcase() -> impl IntoView<()> {
    let draft = State::new(multiline_text_input_seed());
    ...
    Container::new()
        ...
        .child(
            TextInput::new()
                ...
                .width(720.0)
                .height(180.0)
                .key("multiline_text_input-text-input-multiline"),
        )
}
