//! Local-only Phase 50.1 visual tests for TreeView edit/drag states.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_bundle_phase50_1_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_tree_edit_drag_showcase, ShowcaseMode};
...
#[test]
#[ignore]
fn ui_bundle_phase50_1_tree_edit_drag_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-tree-edit-drag");
    let white_id = cap.request_element("white", "section-tree-edit-drag");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_phase50_1_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_tree_edit_drag_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_phase50_1_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_tree_edit_drag_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default tree edit/drag section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white tree edit/drag section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_phase50_1_tree_edit_drag.png");
    assert_non_empty_frame(&white, "ui_bundle_phase50_1_tree_edit_drag_white.png");
    assert_non_monochrome(&default, "ui_bundle_phase50_1_tree_edit_drag.png");
    assert_non_monochrome(&white, "ui_bundle_phase50_1_tree_edit_drag_white.png");
    write_capture("ui_bundle_phase50_1_tree_edit_drag.png", &default);
    write_capture("ui_bundle_phase50_1_tree_edit_drag_white.png", &white);
    ...
    assert!(orange > 30, "orange pixels={orange}");
