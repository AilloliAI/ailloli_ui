//! Local-only terminal diagnostics visual test for Terminal diagnostics.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test terminal_diagnostics_capture_tests -- --ignored --nocapture --test-threads=1
//! ```
...
use ui_bundle_showcase::{ui_bundle_terminal_diagnostics_showcase, ShowcaseMode};
...
/// Verifies the terminal diagnostics overlay, palette contrast, and encoded PNG data.
...
#[test]
#[ignore]
fn ui_bundle_terminal_diagnostics_terminal_diagnostics_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let terminal_id =
        cap.request_element("terminal-diagnostics-suite", "section-terminal-diagnostics");

    App::new()
        .window(
            Window::new("terminal-diagnostics-suite")
                .title_text("ui_bundle_terminal_diagnostics_terminal_diagnostics")
                .no_chrome()
                .size(980.0, 430.0)
                .content(|| ui_bundle_terminal_diagnostics_showcase(ShowcaseMode::DefaultTheme)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    assert_terminal_diagnostics_frame(
        &frame,
        "ui_bundle_terminal_diagnostics_terminal_diagnostics.png",
    );
    write_capture(
        "ui_bundle_terminal_diagnostics_terminal_diagnostics.png",
        &frame,
