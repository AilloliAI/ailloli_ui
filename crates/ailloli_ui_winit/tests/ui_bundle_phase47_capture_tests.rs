//! Local-only Phase 47 visual tests for ComboBox and Autocomplete.
//!
//! ```sh
//! cargo test -p ailloli_ui_winit --test ui_bundle_phase47_capture_tests -- --ignored --nocapture
//! ```
...
use ui_bundle_showcase::{ui_bundle_combobox_autocomplete_showcase, ShowcaseMode};
...
fn assert_non_empty_frame(frame: &CapturedFrame, name: &str) {
    ...
    assert!(frame.height > 120, "{name}: height={}", frame.height);
}
...
#[test]
#[ignore]
fn ui_bundle_phase47_combobox_autocomplete_capture() {
    let cap = CaptureHandle::new();
    cap.set_exit_after_all_captures(true);
    let default_id = cap.request_element("default", "section-combobox-autocomplete");
    let white_id = cap.request_element("white", "section-combobox-autocomplete");

    App::new()
        .window(
            Window::new("default")
                .title_text("ui_bundle_phase47_default")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_combobox_autocomplete_showcase(ShowcaseMode::DefaultTheme)),
        )
        .window(
            Window::new("white")
                .title_text("ui_bundle_phase47_white")
                .no_chrome()
                .size(1280.0, 900.0)
                .content(|| ui_bundle_combobox_autocomplete_showcase(ShowcaseMode::White)),
        )
        .capture(cap.clone())
        .run()
        .expect("app run");
    ...
    let default = cap
        ...
        .expect("default slot")
        .expect("default combobox autocomplete section capture ok")
        .frame;
    let white = cap
        .take_result(white_id)
        .expect("white slot")
        .expect("white combobox autocomplete section capture ok")
        .frame;

    assert_non_empty_frame(&default, "ui_bundle_phase47_combobox_autocomplete.png");
    assert_non_empty_frame(&white, "ui_bundle_phase47_combobox_autocomplete_white.png");
    assert_non_monochrome(&default, "ui_bundle_phase47_combobox_autocomplete.png");
    assert_non_monochrome(&white, "ui_bundle_phase47_combobox_autocomplete_white.png");
    write_capture("ui_bundle_phase47_combobox_autocomplete.png", &default);
    write_capture("ui_bundle_phase47_combobox_autocomplete_white.png", &white);
    ...
    assert!(orange > 10, "orange pixels={orange}");
    ...
    assert!(white_orange > 10, "white orange pixels={white_orange}");
}
