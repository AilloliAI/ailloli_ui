...
use crate::controls::{
    ...
    TreeCreateRequest, TreeDropPosition, TreeMove, TreeMutationMode, TreeNode, TreeShortcut,
...
impl<A: 'static> ComponentNode<A> for FileExplorerComponent<A> {
    fn build(&self, context: &mut Context<A>) -> View<A> {
...

        {
            ...
            let tree_command_for_menu = tree_command.clone();
            let menu_focus_key_for_tree = menu_focus_key.clone();
            let tree_focus_key_for_menu = tree_focus_key.clone();
            tree = tree.on_context_menu_ctx(move |ctx, event| {
                let entries = build_context_menu_entries(
                    ...
                    tree_command_for_menu.clone(),
                    tree_focus_key_for_menu.clone(),
                    event.clone(),
                );
...
fn build_context_menu_entries<A: 'static>(
    ...
    tree_command: Signal<Option<TreeViewCommand<FileUri>>>,
    tree_focus_key: String,
    ...
    ContextMenuEntry::Item(
        ContextMenuItem::new("Rename...")
            ...
            .on_select(ClickAction::handler(move |ctx| {
                tree_command.set(Some(TreeViewCommand::BeginRename(uri.clone())));
